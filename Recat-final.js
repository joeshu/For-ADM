...
if (typeof $response === "undefined") {
  ...
  // 修改：更严格的本地匹配判断 - 确保有有效的 id 或 name
  if (localMatch && (localMatch.name || localMatch.ids)) {
    // 本地匹配成功，直接使用本地数据
    console.log("本地匹配成功:", localMatch.name || "unnamed", localMatch.ids || "no ids");
    processSubscription(localMatch);
  } else {
    // 本地未匹配，从 RevenueCat API 获取 product_entitlement_mapping
    console.log("本地未匹配，尝试从 RevenueCat API 获取 product_entitlement_mapping...");
    fetchProductEntitlementMapping();
  }
} else {
  $done(chxm1024);
}
...
// 从 RevenueCat API 获取 product_entitlement_mapping
function fetchProductEntitlementMapping() {
  ...
  $task.fetch(mappingRequest).then(response => {
    try {
      ...
      if (apiConfig.name && apiConfig.ids && apiConfig.ids.length > 0) {
        console.log("API 获取成功:", apiConfig.name, apiConfig.ids);
        processSubscription(apiConfig);
      } else {
...
// 从 product_entitlement_mapping 提取 name 和 ids（带剔重）
function extractFromMapping(mappingData) {
  ...
  if (!mappingData || !mappingData.product_entitlement_mapping) {
    console.log("mapping 数据无效");
    return result;
  }

  const mapping = mappingData.product_entitlement_mapping;

  // 使用 Set 收集所有唯一的 product_identifiers
  const uniqueProductIds = new Set();

  // 同时收集 entitlement 信息
  let entitlementMap = {}; // entitlement -> Set(product_ids)
    ...
    if (item.entitlements && item.entitlements.length > 0) {
      item.entitlements.forEach(ent => {
        if (!entitlementMap[ent]) {
          entitlementMap[ent] = new Set();
        }
        entitlementMap[ent].add(productId);
      });
    }
  });
  ...
  if (entitlementNames.length > 0) {
    // 使用第一个 entitlement 作为 name
    result.name = entitlementNames[0];
    // Set 转 Array，自动剔重
    result.ids = Array.from(entitlementMap[result.name]);
    result.data = {
      "purchase_date": purchaseDate,
      "expires_date": expiresDate
    };

    // 如果有多个 entitlements，记录额外的（同样剔重）
    if (entitlementNames.length > 1) {
      result.extraEntitlements = entitlementNames.slice(1).map(ent => ({
        name: ent,
        ids: Array.from(entitlementMap[ent])
      }));
    }

    console.log(`从 mapping 提取: name=${result.name}, ids=${JSON.stringify(result.ids)}`);
    console.log(`所有 entitlements: ${JSON.stringify(entitlementNames)}`);
  } else {
    // 没有 entitlements，使用所有唯一的 product_identifiers
    result.name = 'pro';
    result.ids = Array.from(uniqueProductIds);
    result.data = {
      "purchase_date": purchaseDate,
      "expires_date": expiresDate
    };
    console.log(`无 entitlement，使用默认 name=pro, ids=${JSON.stringify(result.ids)}`);
  }
...
function processSubscription(config) {
  ...
  if (name && ids && ids.length > 0) {
    // 主 entitlement - 只创建一个，使用第一个 id
    chxm1023.subscriber.entitlements[name] = Object.assign({}, data || {
      "purchase_date": purchaseDate,
      "expires_date": expiresDate
    }, { product_identifier: ids[0] });

    // 如果有额外的 entitlements（从 mapping 获取的不同 entitlement 名称）
    if (extraEntitlements && extraEntitlements.length > 0) {
      extraEntitlements.forEach(extra => {
        const extraName = extra.name;
        const extraIds = extra.ids;
        if (extraIds && extraIds.length > 0) {
          chxm1023.subscriber.entitlements[extraName] = Object.assign({}, data || {
            "purchase_date": purchaseDate,
            "expires_date": expiresDate
          }, { product_identifier: extraIds[0] });
        }
      });
    }

    // 注意：不再为同一个 name 创建带序号的额外 entitlements（如 Premium_1, Premium_2 等）
    // 所有 ids 都用于 subscriptions/non_subscriptions/other_purchases，但 entitlements 只保留主 name
  ...
  // 设置 subscriptions - 为所有 ids 创建订阅记录
  if (ids && ids.length > 0) {
    ids.forEach(id => {
  ...
  // 设置 non_subscriptions - 为所有 ids 创建非订阅购买记录
  if (ids && ids.length > 0) {
    ids.forEach(id => {
  ...
  // 设置 other_purchases - 为所有 ids 创建其他购买记录
  if (ids && ids.length > 0) {
    ids.forEach(id => {
