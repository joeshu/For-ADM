/**
 * ==========================================
 * Unified VIP Unlock Manager v9.1.2
 * 统一 VIP 解锁管理器 - 修复版 (1+3+4+5+6)
 * @version 9.1.2
 * @description 修复域名索引、复游会稳定性、请求缓存、联通智家调试、V2EX样式
 * ==========================================
 */

[rewrite_local]
# iAppDaily - 余额查询接口（JSON模式-声明式字段设置）
^https:\/\/api\.iappdaily\.com\/my\/balance url script-response-body https://raw.githubusercontent.com/joeshu/For-ADM/refs/heads/master/Unified_VIP_Unlock_Manager_v13_Fixed.js
# TopHub - 账户同步接口（JSON模式-声明式组合）
^https?:\/\/(?:api[23]\.tophub\.(?:xyz|today|app)|tophub(?:2)?\.(?:tophubdata\.com|idaily\.today|remai\.today|iappdaiy\.com|ipadown\.com))\/account\/sync url script-response-body https://raw.githubusercontent.com/joeshu/For-ADM/refs/heads/master/Unified_VIP_Unlock_Manager_v13_Fixed.js
# gps - GPS工具箱（JSON模式-声明式字段设置）
^https:\/\/service\.gpstool\.com\/app\/index\/getUserInfo url script-response-body https://raw.githubusercontent.com/joeshu/For-ADM/refs/heads/master/Unified_VIP_Unlock_Manager_v13_Fixed.js
# kyxq - 口语星球（JSON模式-声明式场景分发）
^https?:\/\/mapi\.kouyuxingqiu\.com\/api\/v2 url script-response-body https://raw.githubusercontent.com/joeshu/For-ADM/refs/heads/master/Unified_VIP_Unlock_Manager_v13_Fixed.js
# mhlz - 魔幻粒子（JSON模式-声明式前缀处理）
^https?:\/\/ss\.landintheair\.com\/storage\/ url script-response-body https://raw.githubusercontent.com/joeshu/For-ADM/refs/heads/master/Unified_VIP_Unlock_Manager_v13_Fixed.js
# v2ex - V2EX去广告（HTML替换模式-修复样式丢失）
^https?:\/\/.*v2ex\.com\/(?!(.*(api|login|cdn-cgi|verify|auth|captch|(\.(js|css|jpg|jpeg|png|webp|gif|zip|woff|woff2|m3u8|mp4|mov|m4v|avi|mkv|flv|rmvb|wmv|rm|asf|asx|mp3|json|ico|otf|ttf))))) url script-response-body https://raw.githubusercontent.com/joeshu/For-ADM/refs/heads/master/Unified_VIP_Unlock_Manager_v13_Fixed.js
# foday - 复游会去广告（多路径模式-修复keyExtractor）
^https?:\/\/apis\.folidaymall\.com\/online\/capi\/component\/getPageComponents url script-response-body https://raw.githubusercontent.com/joeshu/For-ADM/refs/heads/master/Unified_VIP_Unlock_Manager_v13_Fixed.js
# qiujingapp - 球竞APP去广告（多路径模式-声明式清空）
^https?:\/\/gateway-api\.yizhilive\.com\/api\/v2\/index\/carouses\/(3|6|8|11)(\?.*)?$ url script-response-body https://raw.githubusercontent.com/joeshu/For-ADM/refs/heads/master/Unified_VIP_Unlock_Manager_v13_Fixed.js
^https?:\/\/gateway-api\.yizhilive\.com\/api\/v3\/index\/all\?.*position=2.*$ url script-response-body https://raw.githubusercontent.com/joeshu/For-ADM/refs/heads/master/Unified_VIP_Unlock_Manager_v13_Fixed.js
# Keep - 课程/会员接口（正则替换模式）
^https?:\/\/(api|kit)\.gotokeep\.com\/(nuocha|gerudo|athena|nuocha\/plans|suit\/v5\/smart|kprime\/v4\/suit\/sales)\/ url script-response-body https://raw.githubusercontent.com/joeshu/For-ADM/refs/heads/master/Unified_VIP_Unlock_Manager_v13_Fixed.js
# bqwz - 标枪王者游戏数据接口（游戏数值模式）
^https?:\/\/javelin\.mandrillvr\.com\/api\/data\/get_game_data url script-response-body https://raw.githubusercontent.com/joeshu/For-ADM/refs/heads/master/Unified_VIP_Unlock_Manager_v13_Fixed.js
# bxkt - 伴学课堂接口（混合模式-声明式组合）
^https?:\/\/api\.banxueketang\.com\/api\/classpal\/app\/v1 url script-response-body https://raw.githubusercontent.com/joeshu/For-ADM/refs/heads/master/Unified_VIP_Unlock_Manager_v13_Fixed.js
# tv - 影视去广告接口（多路径模式-完全声明式）
^https?:\/\/(yzy0916|yz1018|yz250907|yz0320|cfvip)\..+\.com\/(v2|v1)\/api\/(basic\/init|home\/firstScreen|adInfo\/getPageAd|home\/body) url script-response-body https://raw.githubusercontent.com/joeshu/For-ADM/refs/heads/master/Unified_VIP_Unlock_Manager_v13_Fixed.js
...
^https?:\/\/yr-game-api\.feigo\.fun\/api\/user\/get-game-user-value url script-response-body https://raw.githubusercontent.com/joeshu/For-ADM/refs/heads/master/Unified_VIP_Unlock_Manager_v13_Fixed.js
# 星际使命 - 微信小程序游戏数据修改（JSON声明式处理器-完全重构）
^https?:\/\/star\.jvplay\.cn\/v2\/storage url script-response-body https://raw.githubusercontent.com/joeshu/For-ADM/refs/heads/master/Unified_VIP_Unlock_Manager_v13_Fixed.js
# wohome - 联通智家去广告（条件删除模式-增强调试）
^https:\/\/iotpservice\.smartont\.net\/wohome\/dispatcher url script-response-body https://raw.githubusercontent.com/joeshu/For-ADM/refs/heads/master/Unified_VIP_Unlock_Manager_v13_Fixed.js
# 思朗月影视 - 用户信息VIP解锁
^https?:\/\/theater-api\.sylangyue\.xyz\/api\/user\/info url script-response-body https://raw.githubusercontent.com/joeshu/For-ADM/refs/heads/master/Unified_VIP_Unlock_Manager_v13_Fixed.js
...
const META = {
    name: 'UnifiedVIP',
    version: '9.1.2',
    author: 'joeshu & contributors (Fixed)',
    description: 'Unified VIP Unlock Manager - Fixed v1+3+4+5+6',
    updated: '2026-03-20'
};
...
const ProcessorUtils = {
    ...
    mapArray(arrayPath, fieldMap, condition = null) {
        return function(obj, env) {
            ...
            if (modified > 0 && env) {
                env.debug(`MapArray: modified ${modified}/${arr.length} items in ${arrayPath}`);
                env.markModified(); // 修复2
            }
            return obj;
        };
    },

    // 修复3: filterArray 防御空key + 标记修改
    ...
    clearArray(arrayPath, options = {}) {
        return function(obj, env) {
            ...
            if (Array.isArray(arr)) {
                ...
                arr.length = 0;
                if (env) {
                    env.log(`Cleared ${options.logName || arrayPath}: ${count} items`);
                    if (count > 0) env.markModified(); // 修复2
                }
            }
            return obj;
        };
    ...
    processByKeyPrefix(objPath, prefixHandlers, options = {}) {
        return function(obj, env) {
            ...
            if (hasModified && env) env.markModified(); // 修复2
            return obj;
        };
    },
    ...
    when(condition, processor) {
        return function(obj, env) {
            if (condition(obj, env)) { // 修复5: 传入env用于调试
                return processor(obj, env);
            }
            return obj;
        };
    ...
    deleteFields(...paths) {
        return function(obj, env) {
            ...
            if (deleted > 0 && env) env.markModified(); // 修复2
            return obj;
        };
    }
};
...
const APP_CONFIGS = Object.freeze({
    ...
    // 修复6: V2EX HTML模式改为追加CSS，不替换原有样式
    v2ex: {
        id: 'v2ex',
        name: 'V2EX去广告',
        ...
        htmlReplacements: [
            {
                // 修复6: 在</head>前追加CSS，保留原有样式
                pattern: /<\/head>/i,
                replacement: `<style>body .ads, .ad_banner, #Top .ads, .sidebar_units { display: none !important; }</style></head>`,
                description: '注入CSS隐藏广告元素（追加模式）'
            }
        ]
    },

    // 修复3: 复游会keyExtractor防御空值
    foday: {
        id: 'foday',
        name: '复游会',
    ...
    wohome: {
        ...
        customProcessor: ProcessorUtils.when(
            ...
            (obj, env) => {
                ...
                const shouldProcess = posCode.includes("APP_START_PAGE");
                if (!shouldProcess && env) {
                    env.debug(`[wohome] Skip: posCode="${posCode}" not match "APP_START_PAGE"`);
                } else if (shouldProcess && env) {
                    env.debug(`[wohome] Match: posCode="${posCode}" - will delete configList`);
                }
...
const DomainIndex = (() => {
    ...
    function build(configs) {
        try {
            ...
            for (const [key, config] of Object.entries(configs)) {
                if (!config || !config.urlPattern) continue;
                if (!(config.urlPattern instanceof RegExp)) continue;

                try {
                    const domains = Utils.extractDomainsFromPattern(config.urlPattern);
                    for (const domain of domains) {
                        if (!domain || typeof domain !== 'string') continue;
                        if (_index.has(domain)) continue; // 手动映射优先
                        
                        if (!_index.has(domain)) {
                            _index.set(domain, []);
                        }
    ...
    return {
        ...
        getStats,
        clear,
        isHealthy: () => !_buildError && _totalDomains > 0
    };
})();
...
class Environment {
    constructor(name) {
        ...
        this.hasModified = false; // 修复2: 修改标记
