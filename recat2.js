...
function fetchFromAPIAndProcess() {
    ...
    $task.fetch(myRequest).then(response => {
        try {
            ...
            const apiMatch = extractFromOfferings(offeringsData);
            
            if (apiMatch.name && apiMatch.ids && apiMatch.ids.length > 0) {
                console.log("API 获取成功:", apiMatch.name, apiMatch.ids);
                processSubscription(apiMatch);
            } else {
                console.log("API 未返回有效数据，使用默认配置");
                processSubscription(getDefaultConfig());
            }
        } catch (e) {
            console.log("解析 API 响应失败: " + e.message);
            processSubscription(getDefaultConfig());
        }
    }, reason => {
        console.log("请求 API 失败: " + reason.error);
        processSubscription(getDefaultConfig());
    });
}

// 从 Offerings 数据中提取 name 和 ids
function extractFromOfferings(offeringsData) {
...
function processSubscription(config) {
    ...
    const { name, nameb, ids, idb, data } = config;
