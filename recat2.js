...
// 从 product_entitlement_mapping 提取 name 和 ids
function extractFromMapping(mappingData) {
    ...
    if (productIds.length === 0) {
        console.log("mapping 中无产品数据");
        return result;
    }

    // 收集所有 entitlements 和对应的 product_identifiers
    let entitlementMap = {}; // entitlement -> [product_ids]
    ...
    if (entitlementNames.length > 0) {
        ...
        // 该 entitlement 对应的所有 product_identifiers 作为 ids
        result.ids = entitlementMap[result.name];
        result.data = {
            "purchase_date": purchaseDate,
            "expires_date": expiresDate
        };
        
        // 如果有多个 entitlements，记录额外的
        if (entitlementNames.length > 1) {
            result.extraEntitlements = entitlementNames.slice(1).map(ent => ({
                name: ent,
                ids: entitlementMap[ent]
            }));
        }
        ...
        // 没有 entitlements，使用所有 product_identifiers
        result.name = 'pro';
